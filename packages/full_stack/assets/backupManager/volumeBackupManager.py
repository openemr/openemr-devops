...
def lambda_handler(event, context):
    ...
    parser = argparse.ArgumentParser(
        description="automated hot-snapshot faculty for EC2 volume")
    parser.add_argument("--region", help="ec2 region for volume")
    parser.add_argument("--volume", help="ec2 volume id for backup")
    parser.add_argument(
        "--retain", help="max versions to retain [" + str(defaultRetainCount) + "]")
    parser.add_argument("--description", help="brief note")
    parser.add_argument(
        "--skip", help="reap backups but don't make one", action="store_true")
    ...
    if args.region:
        ...
        localRegion = urllib2.urlopen(
            'http://169.254.169.254/latest/meta-data/placement/availability-zone').read()[:-1]
