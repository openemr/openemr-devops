...
if [[ -f /root/cloud-backups-enabled ]]; then
  ...
  export PASSPHRASE
  duplicity --full-if-older-than 9D /opt/appliance/backups/in boto3+s3://${S3}/Backup
  duplicity remove-all-but-n-full 2 --force boto3+s3://${S3}/Backup
else
  duplicity --no-encryption --full-if-older-than 9D /opt/appliance/backups/in file:///opt/appliance/backups/out
