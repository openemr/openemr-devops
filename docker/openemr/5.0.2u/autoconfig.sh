...
if [ "$REDIS_SERVER" != "" ] &&
   [ ! -f /etc/php-redis-configured ]; then
    ...
    sed -i "s@session.save_handler = files@session.save_handler = redis@" /etc/php7.2/php.ini
    sed -i "s@;session.save_path = \"/var/lib/php/sessions\"@session.save_path = \"tcp://$REDIS_SERVER:6379\"@" /etc/php7.2/php.ini
