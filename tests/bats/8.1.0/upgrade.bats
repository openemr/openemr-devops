# BATS: 8.1.0 upgrade scripts — structure and consistency
...
@test "8.1.0 upgrade: docker-version file exists and is numeric" {
    assert_file_exists "${UPGRADE_DIR}/docker-version"
    run cat "${UPGRADE_DIR}/docker-version"
    [[ $output =~ ^[0-9]+$ ]]
}

@test "8.1.0 upgrade: fsupgrade-1 through fsupgrade-9 exist" {
    for i in 1 2 3 4 5 6 7 8 9; do
        assert_file_exists "${UPGRADE_DIR}/fsupgrade-${i}.sh"
    done
}

@test "8.1.0 upgrade: each fsupgrade has priorOpenemrVersion or echo Start" {
    for i in 1 2 3 4 5 6 7 8 9; do
...
@test "8.1.0 upgrade: fsupgrade-2 runs sql_upgrade" {
    assert_script_contains "${UPGRADE_DIR}/fsupgrade-2.sh" 'sql_upgrade'
}

@test "8.1.0 upgrade: each script has Completed or echo Completed" {
    for i in 1 2 3 4 5 6 7 8 9; do
...
@test "8.1.0 upgrade: scripts use bash or sh" {
    for i in 1 2 3 4 5 6 7 8 9; do
