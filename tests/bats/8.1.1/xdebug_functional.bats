# BATS: 8.1.1 xdebug.sh — functional tests (validation and exit codes)
...
@test "8.1.1 xdebug: exits 1 when XDEBUG_ON and XDEBUG_IDE_KEY both unset" {
    run env -i PATH="$PATH" HOME="${HOME:-/tmp}" bash "$XDEBUG" 2>&1
    assert_failure 1
    assert_output --partial "neither XDEBUG_ON nor XDEBUG_IDE_KEY"
}

@test "8.1.1 xdebug: exits 1 when XDEBUG_ON not 1 and XDEBUG_IDE_KEY empty" {
