# BATS: flex unlock_admin.sh — functional tests
...
@test "flex unlock_admin: exits non-zero when not in container" {
    run bash -c "'$UNLOCK' 2>&1" || true
    [[ $status -ne 0 ]]
}

@test "flex unlock_admin: with one arg, password is passed to php" {
