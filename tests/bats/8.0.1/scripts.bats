# BATS tests for OpenEMR Docker 8.0.1 bash scripts
...
@test "8.0.1: script directory exists" {
    [[ -d "$SCRIPT_DIR" ]]
}

@test "8.0.1: openemr.sh exists and has valid syntax" {
    assert_script_syntax "${SCRIPT_DIR}/openemr.sh"
}

@test "8.0.1: openemr.sh uses bash and sources devtoolsLibrary" {
...
@test "8.0.1: openemr.sh defines OE_ROOT and AUTO_CONFIG" {
    assert_script_contains "${SCRIPT_DIR}/openemr.sh" 'OE_ROOT='
    assert_script_contains "${SCRIPT_DIR}/openemr.sh" 'AUTO_CONFIG='
}

@test "8.0.1: ssl.sh exists and has valid syntax" {
    assert_script_syntax "${SCRIPT_DIR}/ssl.sh"
}

@test "8.0.1: ssl.sh handles self-signed certificate" {
    assert_script_contains "${SCRIPT_DIR}/ssl.sh" 'selfsigned'
}

@test "8.0.1: xdebug.sh exists and has valid syntax" {
    assert_script_syntax "${SCRIPT_DIR}/xdebug.sh"
}

@test "8.0.1: kcov-wrapper.sh exists and has valid syntax" {
    assert_script_syntax "${SCRIPT_DIR}/kcov-wrapper.sh"
}

@test "8.0.1: utilities/unlock_admin.sh exists and has valid syntax" {
    assert_script_syntax "${SCRIPT_DIR}/utilities/unlock_admin.sh"
}

@test "8.0.1: utilities/unlock_admin.sh invokes unlock_admin.php" {
    assert_script_contains "${SCRIPT_DIR}/utilities/unlock_admin.sh" 'unlock_admin.php'
}

@test "8.0.1: upgrade scripts exist and have valid syntax" {
    for i in 1 2 3 4 5 6 7 8 9; do
        assert_script_syntax "${SCRIPT_DIR}/upgrade/fsupgrade-${i}.sh"
    done
}

@test "8.0.1: upgrade scripts have priorOpenemrVersion or echo Start" {
...
@test "8.0.1: devtoolsLibrary.source exists" {
