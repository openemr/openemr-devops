# BATS: 8.0.1 devtoolsLibrary.source — functional tests (source and call prepareVariables)
...
@test "8.0.1 devtoolsLibrary: prepareVariables with custom env sets CONFIGURATION" {
...
@test "8.0.1 devtoolsLibrary: prepareVariables sets CUSTOM_* variables" {
...
@test "8.0.1 devtoolsLibrary: prepareVariables defaults when env empty" {
...
@test "8.0.1 devtoolsLibrary: prepareVariables CUSTOM_ROOT_USER when MYSQL_ROOT_USER set" {
...
@test "8.0.1 devtoolsLibrary: prepareVariables CUSTOM_ROOT_USER defaults to root" {
    run bash -c "export MYSQL_ROOT_PASS=root; source '$LIB'; prepareVariables; echo \"ROOT=\$CUSTOM_ROOT_USER\""
    [[ $status -eq 0 ]]
    [[ $output == *"ROOT=root"* ]]
}

@test "8.0.1 devtoolsLibrary: setGlobalSettings parses OPENEMR_SETTING_ and echoes Set X to Y" {
...
@test "8.0.1 devtoolsLibrary: setGlobalSettings does nothing when no OPENEMR_SETTING_ vars" {
