...
# OpenEMR container type (8.0.1, binary, flex). Used by CI to ensure all
...
#   ./test_functionality.sh 8.0.1 --test fresh_installation
...
usage() {
    ...
    echo ""
    echo "  version    One of: 8.0.1, binary, flex"
    echo ""
...
case "${VERSION}" in
    8.0.1)
        export DOCKERFILE_CONTEXT="${DOCKERFILE_CONTEXT:-${SCRIPT_DIR}/../../docker/openemr/8.0.1}"
        export IMAGE_TAG="${IMAGE_TAG:-openemr:8.0.1-test}"
        ;;
    binary)
    ...
    flex)
        ...
        ;;
    *)
        echo "Error: unknown version '${VERSION}'. Use 8.0.1, binary, or flex."
        usage
        ;;
esac
